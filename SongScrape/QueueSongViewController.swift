...
class QueueSongViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, UISearchBarDelegate {
    ...
    func callApple(url: String) {
        Alamofire.request(url).responseJSON(completionHandler: { response in
            ...
            for item in items {
                ...
                //print("ATWORK URL: \(newArtworkUrl)")
                ...
                let thisSong = Song(title: songTitle, artist: artist, album: album,  artwork: artwork, service: .applemusic, spotifyUri: nil, startTime: 0, appleId: trackId)
    ...
    func callSpotify(url: String) {
        ...
        Alamofire.request(url, headers: headers).responseJSON(completionHandler: { response in
            ...
            for item in items {
                ...
                //print("ARTOWKR URL: \(artworkUrl)")
                let data = NSData(contentsOf: URL(string: artworkUrl)!)
                let artwork = UIImage(data: data as! Data)
                let thisSong = Song(title: songTitle, artist: artist, album: album, artwork: artwork, service: .spotify, spotifyUri: uri, startTime: 0, appleId: nil)
    ...
    func addSongAndReturn(songToSend: Song) {
        ...
        //performSegue(withIdentifier: "SongAdded", sender: nil)
    }
